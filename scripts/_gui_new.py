...
def process_input():
    ...
    if user_query:
        ...
        # Update the last response with the actual response and response time
        st.session_state.conversation_history[-1] = (
            f"Helpbee: {helpbee_response} (Response time: {response_time:.2f} seconds)"
        )
...
def chatbot_gui():
    ...
    with left_column:
        ...
        if st.button("How do I create an account?", key="q1"):
            st.session_state.user_input = "How do I create an account?"
            process_input()  # Process input here only when clicked
        if st.button("How can I track my order?", key="q3"):
            st.session_state.user_input = "How can I track my order?"
            process_input()
        if st.button("What payment methods are accepted?", key="q4"):
            st.session_state.user_input = "What payment methods are accepted?"
            process_input()
        if st.button("How can I cancel my order?", key="q5"):
            st.session_state.user_input = "How can I cancel my order?"
            process_input()
